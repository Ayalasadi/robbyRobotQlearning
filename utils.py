#!/usr/bin/env python3
#utility functions for Q‑learning and epsilon scheduling
import numpy as np
import random
...
def epsilon_for_episode(episode: int, decay_interval: int = 50, initial_epsilon: float = 0.1) -> float:
    ...
    """
    #compute how many full decay intervals have passed
    decay_steps = episode // decay_interval
    #compute epsilon decrement step so that it hits zero after 10 intervals if initial_epsilon is 0.1
...
def select_action(Q: np.ndarray, state: int, epsilon: float, rng: random.Random | None = None) -> int:
    ...
    Returns:
        Action index.
    """
    #use provided RNG or numpy for random choice
    if rng is None:
        rng = random.Random()
    if rng.random() < epsilon:
        return rng.randrange(Q.shape[1])
    #choose the greedy action; break ties randomly
    values = Q[state]
